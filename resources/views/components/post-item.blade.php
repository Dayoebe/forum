    ...
    <div class="bg-white flex flex-col justify-start p-6">
        <div class="flex gap-4">
            @foreach($post->categories as $category)
                <a href={{route('by-category', $category)}} class="hover:text-yellow-500 text-blue-700 text-sm font-bold uppercase pb-4">
                    {{$category->title}}
                </a>
            @endforeach
        </div>
        <a href="{{route('view', $post)}}" class="text-3xl font-bold hover:text-indigo-900 pb-4">
            {{$post->title}}
        </a>
        @if ($showAuthor)
            <p href="{{ route('profile', $post->user->id) }}" class="text-sm pb-3 hover:text-gray-500">
